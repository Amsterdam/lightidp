...
node {
    ...
    stage("Build image") {
        tryStep "build", {
            ...
            def image = docker.build("datapunt/lightidp:${env.BUILD_NUMBER}")
            image.push()
            }
        }
    }
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                docker.withRegistry("${DOCKER_REGISTRY_HOST}",'docker_registry_auth') {
                def image = docker.image("datapunt/lightidp:${env.BUILD_NUMBER}")
                image.pull()
    ...
    node {
        ...
        tryStep "deployment", {
            ...
            parameters: [
                    ...
                    [$class: 'StringParameterValue', name: 'PLAYBOOKPARAMS', value: "-e cmdb_id=app_lightidp"],
                ]
            }
        }
    }
