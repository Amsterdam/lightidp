...
def test_siam_server_error(config, client):
    with responses.RequestsMock() as rsps:
        rsps.add(rsps.GET, config['SIAM_URL'], status=500)
...
def test_get_authn_link(config, client):
    ...
    body = uenc({
        ...
        'as_url': config['SIAM_URL'] + '?request=login1',
        'a-select-server': config['SIAM_A_SELECT_SERVER'],
        'rid': rid,
    })
    expected = '{}?request=login1&a-select-server={}&rid={}'.format(
        config['SIAM_URL'], config['SIAM_A_SELECT_SERVER'], rid
    )
    with responses.RequestsMock() as rsps:
        rsps.add(rsps.GET, config['SIAM_URL'], status=200, body=body)
